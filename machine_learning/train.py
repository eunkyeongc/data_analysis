...
데이터 파일 : machine_learning\ml_data\telecom_churn.csv
...
import json
import joblib   # 학습이 끝난 파이프라인(전처리_모델)을 파일로 저장하고 나중에 다시 불러오기위해 사용. pickle 또는 joblib 사용
import pandas as pd 
...
from sklearn.impute import SimpleImputer    # 결측치를 대표값으로 대체(숫자는 중앙값, 범주형은 최빈값)으로 채운다.
from sklearn.metrics import classification_report, confusion_matrix, roc_auc_score  # 분류에 따른 평가지표들
from sklearn.model_selection import train_test_split    # 학습/평가 데이터 분할
from sklearn.pipeline import Pipeline   # 전처리+모델을 하나로 묶어서 학습/예측에 사용, 코드 재현성을 보장
from sklearn.preprocessing import OneHotEncoder, StandardScaler 

# 데이터 경로 설정
# __file__ : 현재 이 .py 파일 자신의 경로
# .resolve() : 상대경로를 절대경로로 바꾸어 준다.(어디서 실행하든지 안전)
# .parent : 파일이 들어있는 폴더
HERE = Path(__file__).resolve().parent

# C:\Users\Administrator\bigdata2026\data_analysis\machine_learning\ml_data\telecom_churn.csv
DATA_PATH = HERE/"ml_data/telecom_churn.csv"
# print(HERE.parents[0]) # C:\Users\Administrator\bigdata2026\data_analysis
# print(HERE.parents[1]) # C:\Users\Administrator\bigdata2026
# print(HERE.parents[2]) # C:\Users\Administrator
# print(HERE.parents[3]) # C:\Users


# 학습된 모델과 평가지표를 이 파일이 있는 폴도(HERE) 바로 아래에 저장
...
def build_pipeline() -> Pipeline: 
    ...
    numeric_pipe = Pipeline([
        ...
        # 표준화 스케일링 : monthly_fee(수만원 단위), complaints(0~5정도의 단위)처럼 단위차이가 클때 사용
    ...
    category_pipe = Pipeline([
        ...
        # 판다스의 get_dummies()는 그때그때 다른 열을 만든다. VS 사이킷런의 OneHotEncoder는 학습 시점의 규칙을 기억한다. 벡터로 처리, 파이프라인 단위로 할 때 편리하다.
        # handle_unknown='ignore' : 처음 보는 값이 들어오면, 새 컬럼을 만드는 대신 설정에 따라 모든 값이 0인 벡터로 처리한다.
        ('onehot', OneHotEncoder(handle_unknown='ignore'))
    ])

    # --- 컬럼별로 다른 전처리를 동시에 적용 ---
    # ColumnTransformer : " 이 열은 numberic_pip로, 저 열은 category_pipe로 한번에 지정해서 병렬 처리"
    ...
    model = RandomForestClassifier(
        n_estimators=300, 
        class_weight='balanced',  #  이탈(1), 비이탈(0)보다 적은 불균형 데이터 보장
        random_state=42,
...
def train() -> dict:
    ...
    # 입력(X), 정답(y)로 분리하기 --> 한줄로 표현가능 : X, y = data[FEATURES], data[TARGET]
    X = data[FEATURES]
    y = data[TARGET]
    ...
    metrics = {
        ...
        # roc_auc : 0.5(랜덤수준) ~ 1.0(완벽) 사이 값, 임계값(0.5)에 상관없이 모델의 전반적인 분류 성능을 보여주는 지표
        # round(숫자, 4) : 소수 넷째자리까지 반올림
        # float()를 붙여서 형변환을 하는 이유: 파이썬의 기본 float이 아니라, nimpy.float64 타입이라서
        'roc_auc' : round(float(roc_auc_score(y_test, probabilities)), 4), 

        # recall_churn : <재현율> 실제 이탈 고객 중 몇 %를 잡아냈나?  -> 이탈 예측에서는 이 값이 아주 중요!(놓치면 안되는 고객이므로)
