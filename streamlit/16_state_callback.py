...
# 실행방법 : streamlit run streamlit/16_state_callback.py
# ====================================================================================
import streamlit as st

st.title('카운터 (sesstion state  적용)')

# st.session_state :  브라우저 탭(세션) 하나에 묶여서 재실행되어도 값이 사라지지 않는 딕셔너리 형태의 특수 저장소
if 'count' not in st.session_state:
    st.session_state.count = 0
    
def increment():
    """콜백 함수 : 위젯 클릭 시 화면이 다시 그려지기 전 직전에 자동으로 호출되는 함수"""
...
with col1:
    st.button('+ 증가', on_click=increment) # 함수 다음에 () 생략 : 함수 이름만 전달(클릭했을 때만 실행하고 싶을때)
