...
# < 추가 요구사항 > V.3
# - 수강 강좌정보 입력 후 평점평균 확인을 입력하면 평점평균을 출력하되, 평점평균이 4.0이상이면 학기별 장학 대상자임을 출력합니다.
# - 졸업여건 확인을 입력하면 총 등록학기, 총 학점 수, 총 평균평점에 따른 졸업여건 충족 여부를 출력하되, 졸업여건을 모두 충족하고 졸업 평균평점이 4.0이상이면 졸업 장학 대상자임을 출력합니다.
...
def graduate():
    ...
    if tot_grade >= 2.5:
        ...
        print(f'{2.5-tot_grade}평균평점 낮음')   
    
    if tot_semester >= 8 and tot_credit >= 120 and tot_grade >= 4.0:
        print('모든 졸업 요건 충족 !! 졸업 장학 대상자')
    elif tot_semester >= 8 and tot_credit >= 120 and tot_grade < 4.0 and tot_grade >=2.5:
        print('모든 졸업 요건 충족 !! 졸업 대상자')
    else:
        print('졸업 요건 불충족, 졸업 유보')
