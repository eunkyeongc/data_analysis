"""
2026. 08. 07. 
day62_streamlit_subway/pages_src/trend.py

전체 역 기준으로 "일별 추이"와 "요일/주말-평일 패턴"을 함께 보여주는 페이지
...
# --- 주말 vs 평일 시간대별 패턴 미교 ---
st.subheader('평일vs 주말 - 시간대별 패턴 비교')
...
top_stations =(df.groupby('역명', observed=True)['인원수'].sum().nlargest(20).index.tolist())

compare_stations =st.multiselect('비교할 역 선택(인원수 상위 20개 역 중에서, 최대 5개 권장)', options=top_stations, default=top_stations[:3])
