"""
2026. 08. 06.
day62_streamlit_subway/pages_src/home.py
...
picked =st.multiselect('요약에 포함할 역', options=all_stations, default=top10_default)
...
with tab1:
    st.dataframe(summary_df.sort_values('날짜', ascending=False).head(30)[['날짜', '역명', '승하차', '시간대컬럼', '인원수', '요일코드']])

with tab2:
    if not summary_df.empty:
        chart_data =(summary_df.groupby('역명', observed=True)['인원수'].sum().sort_values(ascending=False))
        st.bar_chart(chart_data)

    else:
        st.info("사이드바에서 역을 하나 이상 선택해 주세요.")
