"""
2026. 08. 06. 
day62_streamlit_subway/pages_src/explore.py
...
# 데이터 불러오기
df = load_subway()
station_list = sorted(df['역명'].unique())  #역명을 중복되지 않게 하나씩만 가져와서 오름차순으로 정렬하라.

# 'favorite_stations' 키가 없을때만 빈 리스트로 초기화 --> 이 세션에 처음 방문했을 때만 
...
with col_btn:
    ...
    if st.button('+추가'):
        if station_to_add not in st.session_state.favorite_stations:
            ...
            st.rerun() # 즐겨찾기 목록에 추가한 내용을 화면에 즉시 반영 
...
if not st.session_state.favorite_stations:
    ...
    for i, station in enumerate(st.session_state.favorite_stations):
        ...
        with col_name:
            st.write(f'{i+1}. {station}')  # 출력 예시 : 1.반월당,.....
        with col_del:
            # key=f'del_{i}' : 삭제 버튼이 여러 개일 때 서로 구분하기 위한 고유 key
            if st.button('삭제', key=f'del_{i}'):
                st.session_state.favorite_stations.pop(i) # i번째 값을 꺼내서 삭제
                st.rerun()
...
if st.session_state.favorite_stations:
    ...
    total_by_type = fav_df.groupby('승하차', observed=True)['인원수'].sum()  # observed=True : 실제 존재하는 조합만 그룹한다.
